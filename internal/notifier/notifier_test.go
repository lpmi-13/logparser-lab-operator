...
const testMessageChallengeReady = "challenge-ready"
...
func TestSendEventImmediateDelivery(t *testing.T) {
	...
	n.SendEvent("lab1", Event{Message: testMessageChallengeReady, ChallengeID: "c1"})

	msg, ok := receiveWithTimeout(ch, 100*time.Millisecond)
	if !ok {
		t.Fatal("expected message to be delivered immediately")
	}
	if msg.Message != testMessageChallengeReady {
		t.Errorf("expected %q, got %q", testMessageChallengeReady, msg.Message)
	}
...
func TestSendEventDuplicateSuppression(t *testing.T) {
	...
	event := Event{Message: testMessageChallengeReady, ChallengeID: "c1"}
...
func TestSendEventNotSuppressedAcrossChallenges(t *testing.T) {
	...
	n.SendEvent("lab1", Event{Message: testMessageChallengeReady, ChallengeID: "c2"})
	msg, ok := receiveWithTimeout(ch, 100*time.Millisecond)
	if !ok {
...
func TestSendEventDebouncing(t *testing.T) {
	...
	n.SendEvent("lab1", Event{Message: "challenge-completed", ChallengeID: "c1"})
	...
	if msg.Message != "challenge-completed" {
		t.Errorf("expected %q, got %q", "challenge-completed", msg.Message)
	}
}
